  ...
  <ol aria-label="Long-form posts">
    {#each data.posts as post, index}
      <li style:--offset={`${offsets[index % offsets.length]}vw`}>
        <article>
          <h2><a href={`/long-form/${post.slug}/`}>{post.title}</a></h2>
...
<style>
  ...
  h2 a:hover, h2 a:focus-visible { text-decoration: underline; text-underline-offset: .18em; }
  .short-date { display: block; margin-top: .35rem; font-size: .78rem; opacity: .68; }
  ...
  article:hover .details, article:focus-within .details { grid-template-rows: 1fr; opacity: 1; }
