...
<main>
  ...
  {:else if current}
    <section class="reader" aria-live="polite">
      ...
      <p class="position">{entries.length - currentIndex} / {entries.length}</p>
    </section>
  {:else}
...
<style>
  ...
  .side-arrow { position: sticky; top: 50vh; display: grid; width: 2.5rem; aspect-ratio: 1; place-items: center; margin-top: clamp(5rem, 10vw, 7.5rem); translate: 0 -50%; border: 1px solid rgb(48 43 36 / 35%); border-radius: 50%; background: rgb(242 239 229 / 45%); cursor: pointer; font-size: 1.15rem; transition: background .15s ease, transform .15s ease; }
